...
#for squeezenet backbone
class SqueezeNetBackbone(nn.Module):
    def __init__(self, train_backbone: bool):
        super().__init__()
        # Get pretrained SqueezeNet and remove classifier
        backbone = torchvision.models.squeezenet1_1(weights = torchvision.models.SqueezeNet1_1_Weights.DEFAULT)
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        m = tensor_list.mask
        assert m is not None
        mask = F.interpolate(m[None].float(), size=xs.shape[-2:], mode="nearest").to(torch.bool)[0]
...
def build_backbone(args):
    # Force single-scale (detection only)
    assert not getattr(args, "masks", False), "Set args.masks = False for single-scale detection."
    position_embedding = build_position_encoding(args)
    train_backbone = args.lr_backbone > 0
    backbone = SqueezeNetBackbone(train_backbone)
    model = Joiner(backbone, position_embedding)
    model.num_channels = backbone.num_channels
    return model